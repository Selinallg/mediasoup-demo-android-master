 ...
 * }
 * @TargetApi(21) private VideoCapturer createScreenCapturer() {
...
public class PeerConnectionUtils {
    ...
    private VideoCapturer captureAndroid; // 录屏数据
    private CameraVideoCapturer mCamCapture; // camera 数据

    //  ScreenCapturerAndroid
    private static Intent mMediaProjectionPermissionResultData;

    public static void setMediaProjectionIntent(Intent input_intent) {
        mMediaProjectionPermissionResultData = input_intent;
    }
    ...
    private void createVideoSource(Context context) {
        ...
        if (true) {
            ...
            mCamCapture.startCapture(1280, 720, 30);
        }
        ...
        if (false) {
            ...
            mVideoSource = mPeerConnectionFactory.createVideoSource(captureAndroid.isScreencast());
            SurfaceTextureHelper surfaceTextureHelper = SurfaceTextureHelper.create("CaptureThread", mEglBase.getEglBaseContext());
            // 初始化captureAndroid
            captureAndroid.initialize(surfaceTextureHelper, context, mVideoSource.getCapturerObserver());
            // 摄像头预览的宽度 高度   帧率
            captureAndroid.startCapture(1280, 720, 30);
            //视频轨
//          VideoTrack localVideoTrack = mPeerConnectionFactory.createVideoTrack("video-vr", mVideoSource);
